...
require "MapPosition"
	...
	local E = {Range = 600, Delay = 0.25, Speed = 2500, Radius = 100, Type = line, From = Ekko}
	local R = {Range = 1600, Delay = 0.25, Speed = 1650, Radius = 375}
	...
	local CursorPos = Control.SetCursorPos
...
if self.castAttack.state == 0 and Ekko.pos:DistanceTo(pos.pos) < range and ticker - self.castAttack.casting > delay + Latency() and self:CanAttack() and not pos.dead and pos.isTargetable then
...
if ticker - self.castAttack.tick < Latency() and self.AA.state == 1 then
...
if ticker - self.castAttack.casting > Latency() and self.castAttack.state == 1 then
Control.SetCursorPos(self.castAttack.mouse.x,self.castAttack.mouse.y)
self.castAttack.state = 0
end
end
...
Combo = function()
		...
		--[[	local CastQE = CastSpell
			...
			_G.SDK.Orbwalker:__OnAutoAttackReset(target)
end
end]]--
...
-----------	
	...
	Flux.Combo:MenuElement({id = "UseEQ", name = "EQ", value = true})
